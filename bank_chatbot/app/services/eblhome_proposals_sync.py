"""
Read-only sync helpers for EBL Home proposal updates from MySQL (ebl_home).
"""
...
class EblHomeProposalsSync:
    """Fetch published proposal_update metadata from the ebl_home WordPress MySQL database."""
    ...
    def fetch_proposals(self) -> List[Dict]:
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute(PROPOSALS_QUERY)
            ...
            for row in rows:
                ...
                post_type = self._clean_text(row.get("post_type")) or "proposal_update"
                proposals.append(
                    {
            ...
            logger.info(
                "Fetched %s published proposals from MySQL %s",
                len(proposals),
                self.database,
            )
            return proposals
        except Exception as exc:
            logger.error("Failed to fetch proposals from MySQL: %s", exc, exc_info=True)
            raise
        finally:
...
def get_eblhome_proposals_sync() -> EblHomeProposalsSync:
    return EblHomeProposalsSync()
