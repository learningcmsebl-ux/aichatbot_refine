"""
EBL Home schedule of charge index — searchable metadata and download links only.
"""
...
EBL_SOC_MAX_RESULTS = int(os.getenv("EBL_SOC_MAX_RESULTS", "10"))


class EblSoc(Base):
    __tablename__ = "ebl_soc_index"

    id = Column(Integer, primary_key=True, index=True)
    source_post_id = Column(Integer, unique=True, nullable=False, index=True)
    title = Column(String(500), nullable=False, index=True)
    soc_type = Column(String(255), index=True, nullable=True)
    post_type = Column(String(64), nullable=False, default="schedule_of_charge")
...
class EblSocDB:
    """PostgreSQL index for EBL Home schedule of charge metadata and links."""
    ...
    def _normalize_limit(limit: Optional[int], default: int) -> int:
        ...
        if effective_limit <= 0:
            return 0
        return min(effective_limit, EBL_SOC_MAX_RESULTS)

    @staticmethod
    def _to_dict(item: EblSoc) -> Dict:
        return {
            "id": item.id,
    ...
    def sync_items(self, items: List[Dict], clear_existing: bool = False) -> Dict[str, int]:
        ...
        try:
            with self.get_session() as session:
                if clear_existing:
                    deleted = session.query(EblSoc).delete()
                    session.commit()
                    logger.info("Cleared %s existing SOC records", deleted)

                for item_data in items:
                    try:
                        existing = session.query(EblSoc).filter(
                            EblSoc.source_post_id == item_data["source_post_id"]
                        ).first()
                        if existing:
                            ...
                            session.add(EblSoc(**item_data))
                            stats["inserted"] += 1
                    except Exception as exc:
                        logger.warning(
                            "Failed to sync SOC item %s: %s",
                            item_data.get("title", "unknown"),
                            exc,
                        )
                        stats["errors"] += 1
        except Exception:
            logger.error("SOC sync failed", exc_info=True)
            raise
        return stats
    ...
    def _search_tokens(query: str) -> List[str]:
        stop_words = {
            ...
            "schedule", "charge", "soc", "ebl", "home", "eblhome",
        }
        tokens = [
            token.strip()
        ...
        with self.get_session() as session:
            ...
            if tokens:
                ...
                for token in tokens:
                    ...
                    conditions.append(
                        or_(
                            func.lower(EblSoc.title).like(pattern),
                            func.lower(EblSoc.soc_type).like(pattern),
                        )
                    )
    ...
    def total_items(self) -> int:
        with self.get_session() as session:
            return session.query(EblSoc).count()

    def get_by_source_post_id(self, source_post_id: int) -> Optional[Dict]:
        with self.get_session() as session:
            item = session.query(EblSoc).filter(
                EblSoc.source_post_id == source_post_id
            ).first()
            return self._to_dict(item) if item else None


_ebl_soc_db: Optional[EblSocDB] = None


def get_ebl_soc_db(database_url: Optional[str] = None) -> EblSocDB:
    global _ebl_soc_db
    if _ebl_soc_db is None:
        _ebl_soc_db = EblSocDB(database_url)
    return _ebl_soc_db
